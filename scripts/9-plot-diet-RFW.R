...
frags$RFW <- frags$fragment_weight / as.numeric(frags$dryweight_grams)
frags$pct_RFW <- round(frags$RFW  *100,2)

# calculate relative frequency by weight of plastics 

# plot Figure Supp diet RFW by site 
RFW_by_site <- ggplot(frags, aes(x =fragment_type , y =pct_RFW, fill = tourism_level)) + 
  geom_boxplot() +
...
# vertebrates
t.test(pct_RFW ~ tourism_level, data = frags[frags$fragment_type == "Vertebrate",])
# invertebrates
t.test(pct_RFW ~ tourism_level, data = frags[frags$fragment_type == "Invertebrate",])
# plant
t.test(pct_RFW ~ tourism_level, data = frags[frags$fragment_type == "Plant",])
# anthropogenic
t.test(pct_RFW ~ tourism_level, data = frags[frags$fragment_type == "Anthropogenic",])
